...
function Longbreak(props:{menu:boolean,setMenu:any,font:number,color:number,longBreak:number,setResult:any,result:undefined|string,pause:boolean,setPause:any, setStartInterval:any}){


    useEffect(() => {

        props.setResult(`${minute.current.toString().length == 1?`0${props.longBreak}`:props.longBreak}:00`)
        seconds.current = 0
    ...
    const minute = useRef(props.longBreak)
